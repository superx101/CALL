...
export default class AreaOperation {
    public static start(output: CommandOutput, caPlayer: CAPlayer, res: any) {
        ...
        switch (enum_1) {
            ...
            case "a":
                if (pos != null) {
                    AreaOperation.setPosA(caPlayer, Pos3D.fromPos(pos));
                }
                    ...
                    if (block != null) {
                        AreaOperation.setPosA(caPlayer, Pos3D.fromPos(block.pos));
                    }
                    else {
                    ...
                    AreaOperation.setPosA(caPlayer, Pos3D.fromPos(player.pos).calibration());
                }
                break;
            case "end":
            case "en":
            case "b":
                if (pos != null) {
                    AreaOperation.setPosB(caPlayer, Pos3D.fromPos(pos));
                }
                else if (enum_2 == "view" || enum_2 == "vi") {
                    let max = Config.get(Config.GLOBAL, "viewMaxDistance");
                    let block = player.getBlockFromViewVector(false, false, max, false);
                    if (block != null) {
                        AreaOperation.setPosB(caPlayer, Pos3D.fromPos(block.pos));
                    }
                    else {
                        throw new Error(Tr._(player.langCode, "dynamic.AreaOperation.start.viewError", `${max}`));
                    }
                    ...
                    AreaOperation.setPosB(caPlayer, Pos3D.fromPos(player.pos).calibration());
                }
                break;
            case "clear":
            case "cl":
    ...
    /***private */
    private static checkPos(pos: Pos3D, caPlayer: CAPlayer) {
    ...
    public static setPosA(caPlayer: CAPlayer, pos: Pos3D) {
        AreaOperation.checkPos(pos, caPlayer);
        caPlayer.settings.area.start = Pos3D.fromPos3D(pos).floor();
        AreaOperation.setPos(caPlayer, 1);
    }

    public static setPosB(caPlayer: CAPlayer, pos: Pos3D) {
        AreaOperation.checkPos(pos, caPlayer);
        caPlayer.settings.area.end = Pos3D.fromPos3D(pos).floor();
    ...
    public static showArea(caPlayer: CAPlayer) {
        ...
        if (AreaOperation.hasSetArea(caPlayer)) {
            //结构方块显示
            caPlayer.displayPos = AreaDisplayerManager.set(caPlayer.settings.area, caPlayer);
            return true;
        }
        else {
    ...
    public static hideArea(caPlayer: CAPlayer) {
        if (AreaOperation.hasSetArea(caPlayer)) {
            if (caPlayer.displayPos != null) {
                AreaDisplayerManager.remove(caPlayer);
