...
export default class StructureOperation {
    ...
    public static checkTargetStruct(area: Area3, pos: Pos3, caPlayer: CAPlayer) {
    ...
    public static save(output: CommandOutput, caPlayer: CAPlayer, res: { Name: any; }) {
        ...
        StructureService.save(caPlayer, st, 0, 1, (structid: string, data: Data) => {
            data.saveList[`${structid}`] = st;
            Config.set(Config.STRUCTURES, `private.${player.xuid}`, data);
            StructureService.tp(caPlayer);
            player.sendText(StrFactory.cmdSuccess(Tr._(player.langCode, "dynamic.StructureOperation.save.s4", Area3.fromArea3D(data.saveList[structid].area), structid, name)));
        });
    }
    ...
    public static load(output: CommandOutput, caPlayer: CAPlayer, res: { id: string; IntPos: IntPos; Mirror: string; Degrees: string; IncludeEntities: boolean; IncludeBlocks: boolean; Waterlogged: boolean; Integrity: number; Seed: string; }) {
        ...
        StructureService.undoSave(caPlayer, [new Structure(targetArea)], () => {
            StructureService.load(caPlayer, structure, r.structid, pos, 0, 1, res.Mirror, res.Degrees, res.IncludeEntities, res.IncludeBlocks, res.Waterlogged, res.Integrity, res.Seed, () => {
                StructureService.tp(caPlayer);
    ...
    public static copy(output: CommandOutput, caPlayer: CAPlayer) {
        ...
        //copy
        StructureService.copy(caPlayer, [st], dellast, lastComplex, (complex: Complex) => {
            //存储记录
    ...
    public static paste(output: CommandOutput, caPlayer: CAPlayer, res: { IntPos: Pos; }) {
        ...
        StructureService.undoSave(caPlayer, StructureService.getTargetStructs(data.copy, pos), () => {
