...
import NBTService from "./NBTService";
...
export default class StructureService {
    ...
    /*** private */
    private static canGetBlock(area: Area3): boolean {
    ...
    private static async loadChunk(caPlayer: CAPlayer, area: Area3): Promise<boolean> {
        const player = caPlayer.$;
        const max = Config.get(Config.GLOBAL, "maxLoadCheckNum");
        let n = max;
        return new Promise(resolve => {
            if (StructureService.canGetBlock(area)) {
                resolve(true);
                return;
            }
            else {
    ...
    public static savePos(caPlayer: CAPlayer) {
        caPlayer.prePos = Pos3.fromPos(caPlayer.$.pos).calibration();
    ...
    public static tp(caPlayer: CAPlayer) {
        ...
        const player = caPlayer.$;
        let diArr = Pos3.directionToPosArr(caPlayer.direction);
    ...
    public static async traversal(caPlayer: CAPlayer, areas: Areas, title: string, color: number, successCallback: (x: number, z: number) => Promise<boolean>, overCallback: () => void, failCallback: (x: number, z: number) => void, waitTime: number = Config.get(Config.GLOBAL, "traversalWaitTime")): Promise<void> {
        ...
        for (let x = 0; x < mx; x++) {
            for (let z = 0; z < mz; z++) {
                res = await StructureService.loadChunk(caPlayer, areas[x][z]);
                //加载或执行失败, 退出
    ...
    public static async save(caPlayer: CAPlayer, structure: Structure, index: number, total: number, overCallback: (structid: string, data: any) => void) {
        ...
        await StructureService.traversal(caPlayer, areas, Tr._(player.langCode, "dynamic.StructureManager.save.saving", `${index + 1}/${total}`) , 1, (x: number, z: number) => {
            ...
            NBTService.save(saveid, areas[x][z], false, !caPlayer.settings.saveEntity);
    ...
    public static async load(caPlayer: CAPlayer, preStructure: Structure, structid: string, posInt: Pos3, index: number, total: number, mirror = "none", degrees = "0_degrees", includeEntities = false, includeBlocks = true, waterlogged = false, integrity = 100, seed = "", overCallback: () => void) {
        const player = caPlayer.$;
        let area = Area3.fromArea3D(preStructure.area).relative();
        ...
        let saveid;
        let start;
        await StructureService.traversal(caPlayer, areas, Tr._(player.langCode, "dynamic.StructureManager.load.loading", `${index + 1}/${total}`), 6, (x: number, z: number) => {
            //index变换
            saveid = structid + "_" + x + "_" + z;
            ...
            return Promise.resolve(NBTService.load(caPlayer, saveid, new Pos3(start.x + tx, start.y, start.z + tz, start.dimid), mirror, degreeNum));
        }, () => {
            overCallback();
    ...
    public static getAllNBT(st: Structure, sid: string): NbtCompound[][] {
        ...
        for (let x = 0; x < st.xSize; x++) {
            ...
            for (let z = 0; z < st.zSize; z++) {
                //@ts-ignore
                bnbt = new File(NBTService.PATH + `/${sid + "_" + x + "_" + z}.mcstructure`, FileMode.ReadMode, true)
                    .readAllSync();
                //@ts-ignore
    ...
    public static getTargetStruct(st: Structure, pos: Pos3) {
        let area = Area3.fromArea3D(st.area);
        area = area.relative();
    ...
    public static getTargetStructs(complex: Complex, targetPos: Pos3): Array<Structure> {
        let res: Array<Structure> = [];
        let area;
        Object.keys(complex).forEach((sid, i) => {
            area = Area3.fromArea3D(complex[sid].area).relative();
    ...
    private static async otherSave(caPlayer: CAPlayer, structArr: Array<Structure>, overCallback: (complex: Complex) => void) {
        ...
        for (let i = 0; i < structArr.length; i++) {
            ...
            st.name = "system_save";
            await StructureService.save(caPlayer, st, i, structArr.length, (structid: string) => {
                //成功结束
                //保存文件
                complex[structid] = st;
    ...
    private static async pop(caPlayer: CAPlayer, overCallback: () => void, mod: string) {
        ...
        for (let i = 0; i < keys.length; i++) {
            ...
            await StructureService.load(caPlayer, st, sid, st.area.start, i, keys.length, "none", "0_degrees", false, true, false, 100, "", () => {
                if (i == keys.length - 1) {
    ...
    public static clearCopy(caPlayer: CAPlayer) {
        ...
        if (complex != null) {
            Object.keys(complex).forEach((sid) => {
                StructureService.delete(caPlayer, sid, complex[sid]);
            });
    ...
    public static undoPush(caPlayer: CAPlayer, structArr: Array<Structure>, overCallback: (complex: Complex) => void) {
        ...
        StructureService.otherSave(caPlayer, structArr, (complex: Complex) => {
            ...
            if (undoList.length > Config.get(Config.GLOBAL, "maxUndoStep")) {
                ...
                Object.keys(complex1).forEach((sid) => {
                    StructureService.delete(caPlayer, sid, complex1[sid]);
                });
            }
    ...
    public static undoPop(caPlayer: CAPlayer, overCallback: () => void) {
        StructureService.pop(caPlayer, overCallback, "undo");
    }

    public static redoPop(caPlayer: CAPlayer, overCallback: () => void) {
        StructureService.pop(caPlayer, overCallback, "redo");
    }
    ...
    public static copy(caPlayer: CAPlayer, structArr: Array<Structure>, delLast: any, lastComplex: Complex, overCallback: (complex: Complex) => void) {
        ...
        if (delLast) {
            Object.keys(lastComplex).forEach((sid, i, a) => {
                StructureService.delete(caPlayer, sid, lastComplex[sid]);
            });
        }
        //存储
        StructureService.otherSave(caPlayer, structArr, (complex: Complex) => {
    ...
    public static async paste(caPlayer: CAPlayer, pos: Pos3, data: any, overCallback: () => void) {
        let complex = data.copy;
        const keys = Object.keys(complex);
        for (let i = 0; i < keys.length; i++) {
            const sid = keys[i];
            await StructureService.load(caPlayer, complex[sid], sid, pos, i, keys.length, "none", "0_degrees", false, true, false, 100, "", () => {
                if (i == keys.length - 1) {
                    overCallback();
                }
            });
