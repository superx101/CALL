...
const SHP = {
    ...
    registerPackage: function (name, shapeNames, introduction, icon) {
        this._registerPackage("call.superx101.basicShape", name, shapeNames, introduction, icon);
    },
    Message: {
...
try {
    ll.export(export_cmd, "call_shape_plugin", "call.superx101.basicShape" + "_cmd");
    ll.export(export_form, "call_shape_plugin", "call.superx101.basicShape" + "_form");
    ll.export(export_tutorial, "call_shape_plugin", "call.superx101.basicShape" + "_tutorail");
}
catch (e) {
    if (/'export_cmd'/.test(e.message)) {
        throw new Error("CALL形状包 " + "call.superx101.basicShape" + ": 未定义函数export_cmd");
    }
    else if (/'export_form'/.test(e.message)) {
        throw new Error("CALL形状包 " + "call.superx101.basicShape" + ": 未定义函数export_form");
    }
    else if (/'export_tutorial'/.test(e.message)) {
        throw new Error("CALL形状包 " + "call.superx101.basicShape" + ": 未定义函数export_tutorial");
    }
    else {
        throw new Error(e);
    }
