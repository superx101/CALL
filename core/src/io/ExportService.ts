...
import { NBT, Structure } from "../common/Structure";
...
export default class ExportService {
    public static checkStructure(sid: string): Structure {
        const obj = StructureService.getPrivate();
        let isFind = false;
        let st;
        ...
        if (isFind) return Structure.fromStructure(st);
        else
            throw new Error(
                Tr._c("console.ExportManager.checkStructure.notFind", `${sid}`)
            );
    }
    ...
    public static async toMcstructure(
        st: Structure,
        sid: string,
        includeEntity: boolean = false,
        initProgressCallback: (max: number) => void,
        updateProgresCallback: (current: number) => void
    ): Promise<ByteBuffer> {
        const arr = StructureService.getAllNBT(st, sid);
        const obj0: NBT = arr[0][0].toObject() as NBT;
        const size = Area3.fromArea3D(st.area).getLens();
        const indicesMap = new Map<string, number>();
        const block_indices: NbtInt[][] = [
            new Array(size[0] * size[1] * size[2]).fill(new NbtInt(-1)),
            new Array(size[0] * size[1] * size[2]).fill(new NbtInt(-1)),
        ];
        ...
        for (let ix = 0; ix < mx; ix++) {
            for (let iz = 0; iz < mz; iz++) {
                total += (arr[ix][iz].toObject() as NBT).structure
                    .block_indices[0].length;
            }
        }
        ...
        for (let ix = 0; ix < mx; ix++) {
            for (let iz = 0; iz < mz; iz++) {
                ...
                obj.structure.block_indices[0].forEach((t, i) => {
                    ...
                    if (t != -1) {
                        //单个区域中方块序号转换为全局方块序号
                        ig = ExportService.transform(i, obj.size[2], c);
                        //方块种类判断去重
                ...
                for (let key of block_position_dataT.getKeys()) {
                    ig = ExportService.transform(parseInt(key), obj.size[2], c); //单个区域中序号转换为全局序号
                    block_position_data.setTag(
                        ig.toString(),
                        block_position_dataT.getData(key)
                    ); //复制
                }
        ...
        return new StructureNBT(
            ...
            block_palette,
            block_position_data,
            obj0.structure_world_origin as Vec3Tuple
        ).toBinaryNBT();
    }
